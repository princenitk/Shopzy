...
import { clearErrors, getOrderDetails, updateVendorOrder } from '../../actions/orderAction'; // Use vendor-specific actions
...
const UpdateOrder = () => {
    ...
    useEffect(() => {
        ...
        dispatch(getOrderDetails(params.id)); // Use vendor-specific action
    ...
    const updateOrderSubmitHandler = (e) => {
        ...
        dispatch(updateVendorOrder(params.id, formData)); // Use vendor-specific action
    }

    return (
        <>
            <MetaData title="Vendor: Update Order | Shopzy" />

            {loading ? <Loading /> : (
                <>
                    {order && order.user && order.shippingInfo && (
                        <div className="flex flex-col gap-4">
                            <Link to="/vendor/orders" className="ml-1 flex items-center gap-0 font-medium text-primary-blue uppercase"><ArrowBackIosIcon sx={{ fontSize: "18px" }} />Go Back</Link>
